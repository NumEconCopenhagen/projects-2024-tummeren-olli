...
class SolowModelHumanCapital:
    ...
    def simulate(self):
        ...
        self.K_tilde[0] = self.K[0] / (self.A[0] * self.L[0])
        self.H_tilde[0] = self.H[0] / (self.A[0] * self.L[0])
        ...
        for t in range(p['T'] - 1):
            ...
            growth_denominator = (1 + p['n'])
            if p['endogenous']:
                # In the endogenous case, adjust growth denominator by actual technological change
                growth_denominator *= (self.A[t + 1] / self.A[t])
            ...
            # Update K and H for next period
